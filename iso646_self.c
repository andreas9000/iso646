...
<%
	...
	<%
		...
		return 0;
	%>
	if (argc == 4 and not strcmp(*(argv + 3), "--insert_header"))
	...
	while (fgets(linebuffer, buf_size, fin) not_eq NULL)
	<%
		last_writeout = 0;
		...
		<%
			...
			if (linebuffer<:i:> == '\0') // end of line
				break;
			if (single_quote and linebuffer<:i:> == '\'') // end of single quote literal
				single_quote = 0;
			else if (double_quote and linebuffer<:i:> == '"') // end of double quote literal
				double_quote = 0;
			else if (multicomment and linebuffer<:i:> == '*' and linebuffer<:i + 1:> == '/') // end of multi-line comment
				multicomment = 0;
			else if (not (single_quote or double_quote or multicomment))
			<%
				if (linebuffer<:i:> == '/' and linebuffer<:i + 1:> == '/') // single-line comment, ignore rest of the line
					break;
				if (linebuffer<:i:> == '\'') // entering a single quote literal
					single_quote = 1;
				else if (linebuffer<:i:> == '"') // entering a double quote literal
					double_quote = 1;
				else if (linebuffer<:i:> == '/' and linebuffer<:i + 1:> == '*') // entering a multi-line comment
					multicomment = 1;
				else
				<%
					match = 0;
					...
					<%
						...
						<%
							if (j < 4 or j == 8) // initial tokens which might be followed by a second valid one
							<%
								for (k = 9; k < 16; k++) // range of double-length special tokens
								<%
									if (linebuffer<:i:> == tokens<:k:><:0:> and linebuffer<:i + 1:> == tokens<:k:><:1:>)
									<%
